...
/**
 * Página de Dashboard Seguro (Autenticado)
 * 
 * Lógica de segurança real:
 * - O middleware garante que apenas sessões ativas entrem aqui.
 * - Este componente busca em tempo real os dados de usuário e fatores MFA cadastrados diretamente do Supabase.
 * - Exibe o nível AAL atual da sessão do usuário.
 * - Exibe o tempo restante do token de sessão.
 */
export default function DashboardPage() {
  ...
  // Simular dados adicionais de sessão (IP e localização de rede)
  ...
  const handleLogout = async () => {
    try {
      ...
      // Fazer logout no Supabase (revoga sessão e remove cookies)
      await logoutService();
        ...
        <div className="space-y-4">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-slate-400">
                  Bem-vindo ao sistema seguro (Sessão Ativa)
                </p>
              </div>
            </div>
          ...
          {isExpired && (
            <Alert variant="destructive" className="border-red-500/50 bg-red-500/10">
              ...
              <AlertDescription>
                Sua sessão expirou. Você será redirecionado para efetuar login novamente.
              </AlertDescription>
            </Alert>
          )}
          ...
          {!mfaEnabled && (
            <Alert className="border-yellow-500/50 bg-yellow-500/10">
              ...
              <AlertDescription className="text-yellow-300">
                ...
                <Button variant="link" size="sm" className="text-emerald-400 hover:text-emerald-300 p-0 ml-1 font-semibold underline" onClick={() => router.push('/mfa/setup')}>
        ...
        <div className="grid md:grid-cols-2 gap-6">
          ...
          <Card className="border-emerald-500/20 bg-slate-900/50 backdrop-blur-sm p-6 space-y-4">
            ...
            <div className="space-y-3">
              ...
              <div>
                ...
                <p className="text-sm font-mono text-slate-300 bg-slate-950/40 p-2 rounded border border-slate-800 break-all">
                  {userData.id}
                </p>
              </div>
            </div>
            ...
            <div className="space-y-3">
              ...
              <div>
                ...
                <p className="text-sm text-slate-300 font-mono">
                  {typeof window !== 'undefined' ? navigator.userAgent.substring(0, 50) + '...' : 'Browser Client'}
                </p>
              </div>
          ...
          <Card className="border-blue-500/20 bg-blue-500/5 p-6 space-y-4">
            ...
            <ul className="space-y-2 text-xs text-blue-300 leading-relaxed">
              <li>
                ✓ A autenticação multifator (MFA) diminui drasticamente riscos de vazamento de credenciais.
              </li>
              ...
              <li>
                ✓ Utilize senhas de alta entropia (geradas aleatoriamente com mais de 12 caracteres).
              </li>
            </ul>
          </Card>
        </div>

        {/* Footer com info educacional */}
        <Card className="border-slate-700 bg-slate-800/50 p-4">
          <p className="text-xs text-slate-400 leading-relaxed">
            <span className="font-semibold text-slate-300">
              📚 Nota Acadêmica:
            </span>{' '}
            Este sistema utiliza autenticação com 2 fatores (Senha e TOTP do Google Authenticator). A criptografia de senha (bcrypt + salt) e o gerenciamento de sessões (JWT em cookies HttpOnly/Secure) são realizados no servidor pelo Supabase. O frontend realiza a validação de complexidade com Zod, prevenção de XSS com escape de caracteres e cooldown visual de brute-force.
          </p>
        </Card>
      </main>
    </div>

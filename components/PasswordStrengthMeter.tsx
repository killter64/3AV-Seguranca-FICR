...
/**
 * Componente de Medidor de Força de Senha
 * 
 * Exibe:
 * 1. Barra de progresso colorida (vermelho → amarelo → verde)
 * 2. Nível de força (Fraco, Médio, Forte)
 * 3. Checklist de 5 critérios:
...
export function PasswordStrengthMeter({
  ...
  // Calcular percentagem (5 critérios possíveis)
  const percentage = (score / 5) * 100;

  // Cores baseadas em nível
  const levelColors = {
    weak: 'text-red-500',
  ...
  const criteriaList = [
    ...
    {
      label: 'Número (0-9)',
      met: criteria.numbers,
    },
    {
      label: 'Símbolo especial (!@#$...)',
      met: criteria.symbols,
    },
  ];
    ...
    <div className={cn('space-y-3', className)}>
      ...
      <div className="space-y-1">
        <div className="flex items-center justify-between">
          ...
          <span className={cn('text-sm font-semibold', levelColors[level])}>
            {levelLabels[level]}
          </span>
        </div>
        <Progress
          ...
          aria-label={`Força da senha: ${levelLabels[level]}`}
        />
      </div>
          ...
          <ul className="space-y-1.5">
            {criteriaList.map((criterion) => (
              ...
              >
                ...
                <span
                  className={criterion.met ? 'text-slate-300' : 'text-slate-500'}
                >
                  {criterion.label}
                </span>
              </li>

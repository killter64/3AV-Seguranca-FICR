...
import { enrollMFA, createMFAChallenge, verifyMFACode } from '@/services/totp.service';
...
import { AlertCircle, Loader2, CheckCircle2, Copy, ArrowLeft } from 'lucide-react';
...
export default function MfaSetupPage() {
  ...
  const handleVerifyTotp = async (data: FormValues) => {
    if (!factorId) { setServerError('Fator de autenticação não identificado.'); return; }
    try {
            ...
            {step === 'totp-scan' && (
              <div className="space-y-6">
                <Card className="border-blue-500/20 bg-blue-500/5 p-4 space-y-3">
                  ...
                  <p className="text-xs text-blue-200">2. Abra o aplicativo, adicione uma nova conta e escaneie o código QR abaixo.</p>
                </Card>

                <div className="flex justify-center bg-white rounded-lg p-4 max-w-[240px] mx-auto">
                  {totpUri && <QRCodeSVG value={totpUri} size={200} level="M" includeMargin={true} fgColor="#000000" bgColor="#ffffff" />}
                </div>

                <Card className="border-slate-700 bg-slate-800/50 p-4">
                  <p className="text-xs text-slate-400 mb-2">Não consegue escanear? Digite a chave abaixo manualmente no app:</p>
                  <div className="flex items-center gap-2">
                    <code className="flex-1 break-all bg-slate-900 p-2 rounded text-xs text-emerald-400 font-mono">{totpSecret}</code>
                    <Button size="sm" variant="ghost" onClick={handleCopySecret} className="text-slate-300 hover:bg-slate-800">
                      <Copy className="h-4 w-4" />
                    </Button>
                  </div>
                </Card>

                <Button onClick={() => setStep('totp-verify')} className="w-full bg-emerald-600 hover:bg-emerald-700" size="lg">
            ...
            {step === 'totp-verify' && (
              <div className="space-y-6">
                <Card className="border-emerald-500/20 bg-emerald-500/5 p-4">
                  <p className="text-xs text-emerald-300">Digite o código temporário de 6 dígitos que está sendo gerado no seu aplicativo autenticador.</p>
                </Card>

                <Form {...form}>
                  <form onSubmit={form.handleSubmit(handleVerifyTotp)} className="space-y-6">
                    <FormField control={form.control} name="code" render={({ field }) => (
                      <FormItem>
                        <FormLabel className="text-slate-300 text-center block">Código de Verificação (MFA)</FormLabel>
                        <FormControl>
                          <Input type="text" inputMode="numeric" maxLength={6} placeholder="000000" {...field} disabled={isSubmitting}
                    ...
                    <div className="flex gap-3">
                      <Button type="button" onClick={() => setStep('totp-scan')} variant="outline"
                        className="flex-1 border-slate-700 text-slate-300 hover:bg-slate-800" size="lg" disabled={isSubmitting}>
                        Voltar ao QR Code
                      </Button>
                      <Button type="submit" disabled={isSubmitting || !form.formState.isValid}
                        className="flex-1 bg-emerald-600 hover:bg-emerald-700" size="lg">
